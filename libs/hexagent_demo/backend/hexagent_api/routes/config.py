...
async def put_config(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if "channels_dingtalk" in body:
        dt = body["channels_dingtalk"]
        if dt.get("client_secret", "").startswith("****"):
            dt["client_secret"] = current.channels_dingtalk.client_secret
        current.channels_dingtalk = DingTalkChannelConfig(**dt)
