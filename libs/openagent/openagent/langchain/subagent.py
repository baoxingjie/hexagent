...
:class:`~openagent.types.SubagentRunner` protocol.
...
from openagent.prompts import SUBAGENT_SESSION, compose
from openagent.types import AgentContext, SubagentResult
...
SUBAGENT_EVENT_TAG_PREFIX = "openagent:subagent:"
"""Prefix for tags added to subagent events.

Each subagent invocation is tagged ``"openagent:subagent:<task_id>"``.
...
class LangChainSubagentRunner:
    ...
    async def run(
        ...
        *,
        ...
        if has_skill_tool:
            from openagent.harness.reminders import Reminder as ReminderCls
            from openagent.harness.reminders import available_skills_reminder
        ...
        # 7. Create graph + run
        graph = _create_langchain_agent(sub_profile.model, middleware=[middleware], name="OpenAgent Subagent")
